...
// Router interface for clientapp
type Router interface {
	Route(router *chi.Mux)
}

// ServiceRouter for clientapp API
...
func (s *ServiceRouter) WireRoutes(ctx context.Context, r chi.Router) {
	r.Route(core.SelectBasePath(s.basePathFromSpec, s.gc.BasePath()), func(r chi.Router) {
		...
		r.Get("/bar", s.svcHandler.GetBarListHandler)
		r.Get("/endpoint/{id}", s.svcHandler.GetEndpointHandler)
		r.Get("/foo", s.svcHandler.GetFooListHandler)
		r.Post("/endpoint/{id}", s.svcHandler.PostEndpointWithArgHandler)
	})
}
